...
**Cotizar ítems faltantes:** on a `borrador`, the taller SHALL be able to add missing part names (up to 12 per request, one per line) via `POST /cotizaciones-canal/{id}/cotizar-items/`. The backend MUST append those lines, enrich from taller catalog → historial → web cache, and enqueue web search for remaining `$0` / unverified lines (prioritized over already-priced estimates). UI SHALL show the resulting price and source tag (Catálogo del taller / Historial / Búsqueda web / tienda). Existing lines with catalog/historial price MUST NOT be overwritten. Placeholder names like “Repuesto” MUST NOT be priced.
